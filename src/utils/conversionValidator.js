...
class ConversionValidator {
  ...
  async validateDocxToPdf(docxPath, pdfPath) {
    ...
    try {
      ...
      if (
        pdfStats.size >
        docxStats.size * this.THRESHOLDS.MAX_PDF_SIZE_MULTIPLIER
      ) {
        warnings.push(
          "PDF is significantly larger than source - possible image quality issue"
        );
      }
      ...
      if (wordCount < 10) {
        issues.push(
          "Very little text extracted from PDF - conversion likely failed"
        );
      } else if (wordCount < 50) {
        warnings.push(
          "Low word count detected - verify conversion completeness"
        );
      }
      ...
      if (sizeRatio < 0.1) {
        warnings.push(
          "PDF much smaller than DOCX - fonts may not be properly embedded"
        );
      }

      // Page count estimation (very rough)
      // Average DOCX is ~20KB per page, PDF is ~50KB per page
      const estimatedDocxPages = Math.max(
        1,
        Math.round(docxStats.size / 20480)
      );
      ...
      const pageDiscrepancyPercent =
        (pageDifference / estimatedDocxPages) * 100;

      if (pageDiscrepancyPercent > 20) {
        warnings.push(
          `Page count may be off - estimated ${estimatedDocxPages} pages, got ${actualPdfPages} pages`
        );
      }
      ...
      if (pdfData.info) {
        ...
        if (info.pdfProducer && !info.pdfProducer.includes("LibreOffice")) {
          warnings.push(
            "PDF not produced by LibreOffice - unexpected conversion method"
          );
        }
      }
      ...
      return {
        ...
        info,
        recommendation:
          "Could not complete validation - check PDF file integrity",
      };
    }
  }
    ...
    try {
      ...
      if (expectedData.numpages !== actualData.numpages) {
        ...
        differences.push({
          type: "page_count",
          severity: "high",
          message: `Page count mismatch: expected ${
            expectedData.numpages
          }, got ${actualData.numpages} (${diff > 0 ? "+" : ""}${diff})`,
        });
      }
      ...
      if (wordDiffPercent > 5) {
        differences.push({
          type: "word_count",
          severity: "medium",
          message: `Word count differs by ${wordDiffPercent.toFixed(
            1
          )}%: expected ~${expectedWords}, got ~${actualWords}`,
        });
      }
      ...
      const sizeDiffPercent = Math.abs(
        ((actualSize - expectedSize) / expectedSize) * 100
      );
      ...
      return {
        matching: false,
        differences: [
          { type: "error", severity: "critical", message: error.message },
        ],
        info,
